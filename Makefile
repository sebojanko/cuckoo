...
test: src/tests/main.cpp src/CompactTable.cpp src/FastADocument.cpp src/KMerGenerator/KMerGenerator.cpp src/SimpleEncoder.cpp
	$(CXX) $^ -o test $(CXXFLAGS)

gen: src/KMerGenerator/main.cpp src/FastADocument.cpp src/KMerGenerator/KMerGenerator.cpp
	$(CXX) -O3 $^ -o kmergen $(CXXFLAGS)

bench: src/benchmarks/our_impl_benchmark.cpp src/FastADocument.cpp src/GeneSequence.cpp src/SimpleEncoder.cpp src/CompactTable.cpp
